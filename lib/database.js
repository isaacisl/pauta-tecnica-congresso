...
export function createDatabase(databasePath) {
  ...
  const insertStatement = database.prepare(`
    INSERT INTO records (
      ...
      ha_parecer, sugestao_emenda, posicionamento, camara_json, matter_id
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  function deserialize(row) {
    ...
    return { ...record, camara: camaraJson ? JSON.parse(camaraJson) : null, matter: matter ? { id: matter.id, identifiers: matter.identifiers } : null };
  }
  ...
  function create(input) {
    ...
    const result = insertStatement.run(...recordFields.map((field) => record[field]), input.camara ? JSON.stringify(input.camara) : null, matterId);
  ...
  function update(id, input) {
    ...
    updateStatement.run(...recordFields.map((field) => record[field]), input.camara ? JSON.stringify(input.camara) : null, matterId, id);
    return get(id);
  }
  ...
  function getMatter(id) {
    ...
    return { id: row.id, camara: JSON.parse(row.camara_json), cachedAt: row.cached_at, identifiers };
  }
  ...
  function storeMatter(camara, relations = []) {
    database.prepare(`INSERT INTO legislative_matters(camara_id, camara_json, cached_at) VALUES (?, ?, ?)
      ON CONFLICT(camara_id) DO UPDATE SET camara_json = excluded.camara_json, cached_at = excluded.cached_at`).run(camara.id, JSON.stringify(camara), camara.consultadoEm || new Date().toISOString());
    const { id } = database.prepare("SELECT id FROM legislative_matters WHERE camara_id = ?").get(camara.id);
    const identifiers = [{ source: "camara", externalId: camara.id, house: "CD", siglaTipo: camara.siglaTipo, numero: camara.numero, ano: camara.ano }, ...relations.flatMap((relation) => relation.identifiers.map((identifier) => ({ ...identifier, evidence: relation.evidence })))];
    for (const identifier of identifiers) {
      ...
      if (old && (old.matter_id !== id || old.house !== identifier.house || old.sigla !== identifier.siglaTipo || old.numero !== identifier.numero || old.ano !== identifier.ano)) {
        throw Object.assign(new Error("As fontes retornaram um vínculo conflitante com uma equivalência já salva. Nenhuma equivalência foi alterada."), { status: 409 });
      }
  ...
  function resolveMatter(input) {
    if (!input.camara?.id) return null;
    return (getMatterByCamaraId(input.camara.id) || saveMatter(input.camara)).id;
  }
