...
export function createDatabase(databasePath) {
  ...
  function groupedBy(column) {
    return database
      .prepare(`SELECT ${column} AS label, COUNT(*) AS count FROM records GROUP BY ${column} ORDER BY count DESC, label COLLATE NOCASE`)
      .all();
  }

  function totals() {
    return {
      total: Number(database.prepare("SELECT COUNT(*) AS count FROM records").get().count),
      byArea: groupedBy("area_tecnica"),
      byParecer: groupedBy("ha_parecer"),
      byEmenda: groupedBy("sugestao_emenda"),
      byPosicionamento: groupedBy("posicionamento")
    };
  }
