...
export function createCamaraClient(fetchImpl = fetch) {
  ...
  async function select(id, searchToken) {
    ...
    const proposition = await detail(id);
    if (!matches(proposition, query)) throw unavailable();
    return { proposition, propositionToken: sign("selection", proposition) };
  }
  async function detail(id) {
    if (!Number.isSafeInteger(id) || id < 1) throw searchRequired();
    const { dados } = await get(`proposicoes/${id}`);
    if (!dados || dados.id !== id || !allowedTypes.has(dados.siglaTipo) || !Number.isSafeInteger(dados.numero) || !Number.isInteger(dados.ano)) throw unavailable();
    const proposition = {
      id, source: "camara", sourceUrl: `${baseUrl}proposicoes/${id}`,
      siglaTipo: dados.siglaTipo, numero: dados.numero, ano: dados.ano,
      projeto: `${dados.siglaTipo} ${dados.numero}/${dados.ano}`,
      ...
      statusDescricao: dados.statusProposicao?.descricaoSituacao || null,
      statusTramitacao: dados.statusProposicao?.descricaoTramitacao || null,
  ...
  return { search, select, detail, recordInput };
}
